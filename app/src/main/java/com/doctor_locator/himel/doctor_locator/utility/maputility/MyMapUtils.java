package com.doctor_locator.himel.doctor_locator.utility.maputility;
...
public class MyMapUtils implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener {
    ...
    public MyMapUtils(Context mContext, GoogleMap googleMap) {

        this.googleMap = googleMap;
        this.mContext = mContext;
    }

    public void getDirectionOnMap(double destLtd,double destLng){
        ...
        markerPoints.add(new LatLng(AppConstant.MY_LOCATION_LATITUTE,AppConstant.MY_LOCATION_LONGITUTE));
        markerPoints.add(new LatLng(destLtd,destLng));
    ...
    private class ParserTask extends AsyncTask<String, Integer, List<List<HashMap>>> {
        ...
        protected void onPostExecute(List<List<HashMap>> result) {
            ...
            for (int i = 0; i < result.size(); i++)
            {
                ...
                lineOptions.width(8);
                lineOptions.color(ContextCompat.getColor(mContext, R.color.colorAccent));
                lineOptions.geodesic(true);

            }
            if (lineOptions==null){
                Toast.makeText(mContext, "path not found", Toast.LENGTH_LONG).show();
                return;
            }
